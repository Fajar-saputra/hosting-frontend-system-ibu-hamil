    ...
    const ArticleForm = () => {
        ...
        useEffect(() => {
            const fetchData = async () => {
                try {
                    ...
                    const resDiseases = await axios.get(`${API_BASE}/api/penyakit`);
                    setDiseases(resDiseases.data);

                    // Jika mode edit, fetch data artikel berdasarkan ID
                    if (isEditMode) {
                        const { data } = await axios.get(`${API_BASE}/api/articles/edit/${id}`);
                        setTitle(data.title);
                        setSlug(data.slug);
        ...
        const handleSubmit = async (e) => {
            ...
            try {
                ...
                if (isEditMode) {
                    // Update pakai PUT
                    await axios.put(`${API_BASE}/api/articles/${id}`, formData, config);
                    alert("Artikel berhasil diperbarui!");
                } else {
                    // Create pakai POST
                    await axios.post(`${API_BASE}/api/articles`, formData, config);
